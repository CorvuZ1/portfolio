        ...
        <div className="grid gap-4">
          {contacts.map((contact) => (
            <a
              ...
              href={contact.href}
              target="_blank"
              className="group flex flex-col items-start gap-4 rounded-[1.75rem] border border-white/10 bg-white/6 px-6 py-5 backdrop-blur-xl transition hover:border-cyan-300/35 hover:bg-white/8 sm:flex-row sm:items-center sm:justify-between"
            >
              <span>
                <span className="block text-xs uppercase tracking-[0.3em] text-slate-500">
                  {contact.label}
                </span>
                <span className="mt-3 block text-xl font-semibold text-white">{contact.value}</span>
              </span>
              <span className="rounded-full border border-white/10 px-4 py-2 text-sm text-slate-300 transition group-hover:border-cyan-300/35 group-hover:text-white">
                Lorem
              </span>
            </a>
          ))}
